...
RETURNS_COLUMN = "Mean Nominal Return (%)"
...
NUM_RESTARTS = 15
MAXITER = 50000
USE_BOUNDS = False               # Set False to disable bounds
DENSITY_FLOOR = 1e-12
VERBOSE = True
...
DEFAULT_BOUNDS = [
    ...
    (1e-12, 50.0),    # theta (annual var)
    (1e-6, 50.0),     # nu (vol-of-vol annual)
    (-0.999, 0.999),  # rho
    (1e-12, 50.0),    # v0 (annual var)
    (0.0, 200.0),     # lambda (jumps per year)
    (-20.0, 20.0),    # mu_J (jump mean, log-return)
    (1e-12, 20.0),    # sigma_J (jump std)
]
...
def plot_empirical_vs_bates(returns, params, regime_id, save_fig=False):
    ...
    if save_fig:
        ...
        plt.savefig(f"bates_regime_{safe}.png", dpi=150)
    plt.show()
